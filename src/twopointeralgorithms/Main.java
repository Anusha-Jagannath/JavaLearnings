...
public class Main {
    ...
    public static void main(String args[]) {
        ...
        int n = arr.length;
        System.out.print(findTwoSum(arr, n, 0));
        //System.out.print(findTriplets(arr,n,0));

        boolean isFound = false;
        for (int i=0;i<n-2;i++) {
            if(findTriplets(arr,i,n-1,-arr[i])) {
                isFound = true;
                break;
            }
        }
    ...
    private static boolean findTwoSum(int arr[], int n, int sum) {
        for (int i = 0; i < n; i++) {
    ...
    private static boolean findTriplets(int arr[], int i,int j, int sum) {
       // int i = 0;
        //int j = n - 1;
